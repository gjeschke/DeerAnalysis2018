function handback=fit_user_model(handles)
%
...
if confit,
    ...
    constrainfit=0;
    mcon=struct('lb',[],'ub',[]);
    [sp,lb,ub,Ic]=deal([]);  %initialize start parameter values, lower bounds, upper bounds, constraint indices
    ...
    switch handles.user_model
        case {'Two_Gaussians','Two_Rice3d'}
            ...
            Ic=Ip(find(handles.model_mask(Ip)));  %constrained parameter indices
            if length(Ic)>0
...
if constrainfit % *** HC Hyde
    ...
    if kcurrent<mcon.lb | kcurrent>mcon.ub
        if handles.model_mask(k1)==1 & handles.model_mask(k2)==1  %<r1> fitted, <r2> fitted
            ...
            else aa=abs(kcurrent-mcon.lb)/kcurrent;
            end
            if ~isempty(aa)
